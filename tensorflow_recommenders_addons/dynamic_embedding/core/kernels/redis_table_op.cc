...
class RedisTableOfTensors final : public LookupInterface {
 ...
 private:
  ...
  void launchAccum_parallel(OpKernelContext *ctx,
    ...
    auto shard = [this, &ctx, &total, &keys_prefix_name_slices, &keys, &values_or_delta,
                  &Velems_per_flat2_dim0,
                  &threads_Insert](int64 begin, int64 end) {
      const int64 max_i = std::min(total, end);
      ...
      OP_REQUIRES_OK(
          ctx, launchAccumCore(_table_instance, keys_prefix_name_slices, keys,
                                values_or_delta, Velems_per_flat2_dim0, threads_Insert,
 ...
 public:
  ...
  Status DoAccum(OpKernelContext *ctx, const Tensor &keys,
                  const Tensor &values_or_delta) {
    int64 total = keys.NumElements();
    const int64 Velems_per_flat2_dim0 =
        values_or_delta.NumElements() / keys.NumElements();

    if (total < (multi_redis_cmd_max_argc - 1)) {
      launchInsert(ctx, keys_prefix_name_slices, keys, values_or_delta, total,
                   Velems_per_flat2_dim0, threads_Insert);
    } else {
      launchInsert_parallel(
          ctx, keys_prefix_name_slices, keys, values_or_delta, total,
          Velems_per_flat2_dim0,
          threads_Insert);  // redis commmand args > multi_redis_cmd_max_argc
    }
  ...
  Status Accum(OpKernelContext *ctx, const Tensor &keys,
                const Tensor &values_or_delta) {
    return DoAccum(ctx, keys, values_or_delta);
  }
...
class HashTableAccumOp : public HashTableOpKernel {
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    OP_REQUIRES_OK(ctx,
                   redisTable->Accum(ctx, keys, values_or_deltas));
    if (ctx->track_allocations()) {
      ctx->record_persistent_memory_allocation(table->MemoryUsed() -
                                               memory_used_before);
    }
...
#define REGISTER_KERNEL(key_dtype, value_dtype)                             \
  ...
  REGISTER_KERNEL_BUILDER(Name(PREFIX_OP_NAME(RedisTableAccum))             \
          .Device(DEVICE_CPU)                                               \
          .TypeConstraint<key_dtype>("key_dtype")                           \
          .TypeConstraint<value_dtype>("value_dtype"),                      \
